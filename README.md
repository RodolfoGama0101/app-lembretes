...
Lembretes é um aplicativo Flutter de lembretes locais. Ele registra tarefas em poucos toques, sem conta, servidor ou coleta de dados.
...
> O Android 14 ou posterior pode permitir dispensar uma notificação permanente pelo painel, mesmo com `ongoing: true`. O aplicativo Lembretes a restaura ao abrir ou retomar o app. No iOS, a notificação também pode ser dispensada pelo sistema.
...
Na primeira criação de lembrete, permita notificações. Para alertas temporários no Android, permita também alarmes exatos. Se esse acesso não for concedido, o aplicativo Lembretes usa um agendamento aproximado. Notificações permanentes aparecem imediatamente e não precisam dessa permissão de alarme.

### Prévia local no navegador

`flutter run -d chrome` abre o aplicativo Lembretes no Chrome após `flutter pub get`. Os lembretes ficam no armazenamento local desse navegador; limpar os dados do site os remove. A prévia Web não envia notificações, inclusive quando a aba está fechada. Para testar alertas, use Android ou iOS.
...
A arte aprovada e a camada transparente ficam em assets/branding/. O gerador
exporta os PNGs de Android, iOS e Web e uma prévia dos recortes:

    python -m pip install -r tool/requirements.txt
    python tool/generate_app_icons.py

Os identificadores internos fio_lembretes permanecem para preservar instalações
e dados locais existentes.
